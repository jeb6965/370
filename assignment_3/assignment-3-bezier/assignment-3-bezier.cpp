...
int main() {
	for(int set = 1; set < 3; set++){
		...
		std::string control_points_bezier = "control_points_bezier" + std::to_string(set) + ".jpg";
		canvas.save(control_points_bezier.c_str());
		float increment;
		if (set == 1){
			increment = 0.01;
		}
		...
		for (float step = 0.0f; step <= 1.0f; step += increment){
			std::cout << "Step: "<< step << set << std::endl;
		...
		std::string bezier_final = "Bezier"+ std::to_string(set) + ".jpg";
		canvas.save(bezier_final.c_str());
